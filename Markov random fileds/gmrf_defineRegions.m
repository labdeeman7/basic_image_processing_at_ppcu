function mrf = gmrf_defineRegions(params, mrf)
    ...
    for ind=1:params.NumOfClasses
        ...
        % ====================================== %
        r = rects{ind};
        cropped_image = double( params.InputImage( r.y1:r.y2, r.x1:r.x2 ) );
        
        mrf.gauss_params{ind}.mean = mean( cropped_image(:) );
        mrf.gauss_params{ind}.dev = std( cropped_image(:) );
        
        input_image = double(params.InputImage);
        
        mrf.logProbs{ind} = - log( normpdf( input_image, ...
            mrf.gauss_params{ind}.mean, mrf.gauss_params{ind}.dev ) );
